...
- `vpn-server.sh` ставит 3X-UI, сохраняет credentials и открывает базовые публичные порты, а TLS/WARP/routing rules настраиваются вручную
...
После install откройте `/root/.vpn-server-credentials`: там будет URL панели из install flow, текущий логин и подсказки по портам. Настройки панели (`port`, `webBasePath`, TLS certificate, subscription path и routing rules) теперь меняются вручную в 3X-UI.
...
- после завершения официального install flow больше не трогает `x-ui`
- открывает в `ufw` порты `80/tcp`, `443/tcp` и `65000/tcp`
- сохраняет итоговую служебную информацию в `/root/.vpn-server-credentials`

Что остается ручным:

- смена panel port / web base path / логина / пароля
- выпуск и привязка TLS-сертификата для панели / подписок
- открытие subscription port в firewall, если вы используете отдельный публичный порт подписок
- настройка subscription port/path в самой панели
- установка и запуск WARP proxy на уровне ОС
- создание outbound и routing rules в 3X-UI под WARP

Ручной WARP pipeline и порядок дальнейшей настройки описаны в `docs/CONFIG_PIPELINE.md`.
