...
- `vpn-server.sh` подготавливает self-signed HTTPS для панели, а WARP и routing rules настраиваются вручную
...
После install откройте `/root/.vpn-server-credentials`: там будет фактический URL панели, текущий логин, подсказки по портам и путь к self-signed сертификату. Настройки панели (`port`, `webBasePath`, subscription path и routing rules) теперь меняются вручную в 3X-UI.
