...
- `vpn-server.sh` подготавливает self-signed HTTPS для панели и пытается поднять WARP proxy на `127.0.0.1:40000`
...
После install откройте `/root/.vpn-server-credentials`: там будет фактический URL панели, текущий логин, WARP proxy и путь к self-signed сертификату. Настройки панели (`port`, `webBasePath`, subscription path и routing rules) теперь меняются вручную в 3X-UI.
...
- скачивает `menu.sh`, пытается установить WARP client и поднять proxy на `127.0.0.1:40000`
...
- WARP в `vpn-server.sh` работает в best-effort режиме
- если установка/подключение WARP не удались, bootstrap ноды не падает
- итоговое состояние WARP нужно проверять отдельно через `warp-cli` или `curl -x socks5h://127.0.0.1:40000 https://cloudflare.com/cdn-cgi/trace`
