...
- вручную поднять WARP proxy на ноде
...
- скачивает `menu.sh`
- запускает установку клиента через `bash menu.sh c`
- поднимает локальный SOCKS5 proxy на `127.0.0.1:40000`
- проверяет, что Cloudflare видит `warp=`
