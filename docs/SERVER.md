...
- `ops/vpn-node/vpn-server.sh` — подготовка VPN-ноды с 3X-UI, self-signed HTTPS и WARP proxy в best-effort режиме
...
- скачать `menu.sh` из `fscarmen/warp`
- попытаться установить WARP client и поднять proxy на `127.0.0.1:40000`
...
- не прерывает bootstrap, если WARP-установка или запуск не удались
...
`vpn-server.sh` пытается прогнать такой pipeline:
...
Но этот блок работает в best-effort режиме:

- если клиент уже установлен, скрипт не падает
- если `menu.sh` возвращает ошибку, bootstrap продолжается
- если проверка `warp=` не удалась, bootstrap тоже продолжается

После установки все равно желательно проверить WARP вручную.
