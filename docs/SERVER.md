...
- `ops/vpn-node/vpn-server.sh` — подготовка VPN-ноды с 3X-UI и self-signed HTTPS
...
- не устанавливает и не настраивает WARP
...
WARP полностью вынесен из `vpn-server.sh`.

Если он нужен, устанавливайте и настраивайте его вручную после bootstrap ноды.
...
- `/root/.vpn-server-credentials` — фактический URL панели, логин и пути к сертификату
